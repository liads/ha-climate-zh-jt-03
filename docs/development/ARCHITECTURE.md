...
This document describes the technical architecture of the Climate for IR Devices using ZH/JT-03 Remote custom component for Home Assistant.
...
│   ├── __init__.py          # Exports ClimateZHJT03DataUpdateCoordinator
...
│   ├── __init__.py          # Exports ClimateZHJT03Entity
...
- `base.py` - Main coordinator class (`ClimateZHJT03DataUpdateCoordinator`)
...
**Key class:** `ClimateZHJT03DataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `ClimateZHJT03ApiClient`
...
- `ClimateZHJT03ConfigFlowHandler` (main flow)
- `ClimateZHJT03OptionsFlow` (options)
...
**Key class:** `ClimateZHJT03Entity` (in `entity/base.py`)
...
2. `ClimateZHJT03Entity` for common functionality
...
1. Create directory: `custom_components/climate_ir_zhjt03/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `ClimateZHJT03Entity`
