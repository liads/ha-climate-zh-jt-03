"""Tests for the ZH/JT-03 climate entity."""
...
from .common import UnitOfTemperature, clear_integration_modules, fake_config_entry, install_homeassistant_stubs
...
def climate(monkeypatch):
    ...
    return importlib.import_module(
        "custom_components.climate_ir_zhjt03.climate.zh_jt_03",
    )


def _entity(climate, **data_overrides):
    """Create a ZH/JT-03 climate entity."""
    entity = climate.ClimateZHJT03(fake_config_entry(climate, **data_overrides))
    entity.entity_id = "climate.living_room"
    return entity


def _state(state: str, **attributes):
    """Create a tiny state object."""
...
def _event(new_state):
    """Create a tiny state-changed event."""
    return SimpleNamespace(data={"new_state": new_state})


def test_setup_entry_adds_entity(monkeypatch) -> None:
    """Platform setup creates one climate entity."""
    install_homeassistant_stubs(monkeypatch)
    clear_integration_modules(monkeypatch)
    platform = importlib.import_module("custom_components.climate_ir_zhjt03.climate")
    added = []

    asyncio.run(
        platform.async_setup_entry(
            None,
            fake_config_entry(platform),
            added.extend,
        ),
    )

    assert len(added) == 1
    assert isinstance(added[0], platform.ClimateZHJT03)
...
def test_restore_ignores_invalid_temperature(climate) -> None:
    ...
    assert entity._attr_swing_mode == "fast"
...
def test_setters_defer_to_apply_state_or_update_offline_preferences(climate) -> None:
    ...
    asyncio.run(entity.async_set_swing_mode("slow"))
    assert entity._attr_swing_mode == "slow"
