"""Shared test helpers for Climate for IR Devices using ZH/JT-03 Remote."""
...
class TemperatureConverter:
    ...
    def convert(value: float, from_unit: str | None, to_unit: str | None) -> float:
        ...
        if from_unit == UnitOfTemperature.FAHRENHEIT and to_unit == UnitOfTemperature.CELSIUS:
            return (value - 32) / 1.8
        if from_unit == UnitOfTemperature.KELVIN and to_unit == UnitOfTemperature.CELSIUS:
            return value - 273.15
...
def clear_integration_modules(monkeypatch) -> None:
    ...
    for name in list(sys.modules):
        if name == "custom_components.climate_ir_zhjt03" or name.startswith(
...
def install_homeassistant_stubs(monkeypatch) -> None:
    ...
    redact.async_redact_data = _redact_data
...
def _redact_data(data: Any, keys: set[str]) -> Any:
    ...
    if isinstance(data, dict):
        return {key: "**REDACTED**" if key in keys else _redact_data(value, keys) for key, value in data.items()}
    return data


def fake_config_entry(module, **data_overrides: Any):
    """Return a fake config entry for the climate entity."""
    name_key = getattr(module, "CONF_NAME", "name")
    infrared_key = getattr(module, "CONF_INFRARED_ENTITY_ID", "infrared_entity_id")
    data = {
        name_key: "Living Room",
        infrared_key: "infrared.remote",
    }
